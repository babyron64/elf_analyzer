...
static int eval_rela_read(int cmdc, int ix, char cmds[][MAX_CMD_LEN]);

int
eval_rela(int cmdc, int ix, char cmds[][MAX_CMD_LEN]) {
    if (cmdc == 0) {
    ...
    if (strcmp(cmd, "read") == 0)
        return CMD_CALL(eval_rela_read, cmdc, ix, cmds);
...
static int
eval_rela_read(int cmdc, int ix, char cmds[][MAX_CMD_LEN]) {
    int ndx = 0;
    int str_ndx = 0;
    if (cmdc == 0) {
